...
AJM.moduleDisplayName = L["SellGreysTitle"]

-- Settings - the values to store and their defaults for the settings database.
AJM.settings = {
	profile = {
		deleteItemOnAllWithAltKey = false,
...
AJM.COMMAND_SELL_ITEM = "JmbSelIt"
AJM.COMMAND_DELETE_ITEM = "JmbDelIt"
AJM.CONFIRM_DELETE_ITEM = "JmbDelItCn"
...
function AJM:SettingsRefresh()
	...
	AJM.settingsControlGreys.checkBoxDeleteItemOnAllWithAltKey:SetValue( AJM.db.deleteItemOnAllWithAltKey )
	-- Messages.
...
function AJM:JambaOnSettingsReceived( characterName, settings )
	if characterName ~= AJM.characterName then
		...
		AJM.db.deleteItemOnAllWithAltKey = settings.deleteItemOnAllWithAltKey
		...
		AJM:SettingsRefresh()
		-- Tell the player.
		AJM:Print( L["PushRecieved"]( characterName ) )
	end
end
...
local function SettingsCreateGreys( top )
	...
	local movingTop = top
	JambaHelperSettings:CreateHeading( AJM.settingsControlGreys, L["AltSellDeleteHeading"], movingTop, false )
	...
	AJM.settingsControlGreys.checkBoxDeleteItemOnAllWithAltKey = JambaHelperSettings:CreateCheckBox(
		AJM.settingsControlGreys,
		headingWidth,
		left,
		movingTop,
		L["AltDeleteOnTeam"],
		AJM.SettingsToggleDeleteItemOnAllWithAltKey
	)
	movingTop = movingTop - checkBoxHeight
	...
	JambaHelperSettings:CreateHeading( AJM.settingsControlGreys, L["SellGreysHeading"], movingTop, false )
	movingTop = movingTop - headingHeight
	...
	JambaHelperSettings:CreateHeading( AJM.settingsControlGreys, L["SellException"], movingTop, false )
	movingTop = movingTop - headingHeight
...
local function SettingsCreateOthers( top )
	...
	local movingTop = top
	JambaHelperSettings:CreateHeading( AJM.settingsControlOthers, L["SellOthersTitle"], movingTop, false )
	movingTop = movingTop - headingHeight
	...
	movingTop = movingTop -	buttonHeight - verticalSpacing
	JambaHelperSettings:CreateHeading( AJM.settingsControlOthers, L["AddOther"], movingTop, false )
	movingTop = movingTop - headingHeight
...
function AJM:SettingsToggleDeleteItemOnAllWithAltKey( event, checked )
	AJM.db.deleteItemOnAllWithAltKey = checked
	AJM:SettingsRefresh()
end
...
function AJM:SettingsEditBoxChangedGreyExceptionTag( event, text )
	if not text or text:trim() == "" or text:find( "%W" ) ~= nil then
		AJM:Print( L["ExceptionItemTagName"] )
		return
	end
...
local popupLink = nil
local popupDeleteOnToon = false
function AJM:PopupToonDelete( event, name, link )
	popupLink = link
	popupName = name
	if popupDeleteOnToon == true then
		StaticPopup_Hide( "JAMBASELL_CONFIRM_DELETE_TOON_ITEMS" )
		StaticPopup_Show( "JAMBASELL_CONFIRM_DELETE_TOONS_ITEMS" )
	else
		StaticPopupDialogs["JAMBASELL_CONFIRM_DELETE_TOON_ITEMS"].text = L["AltDeleteItemsOnToonPopup"](popupName)
		StaticPopup_Show( "JAMBASELL_CONFIRM_DELETE_TOON_ITEMS" )
		popupDeleteOnToon = true
	end
end
...
local function InitializePopupDialogs()
	StaticPopupDialogs["JAMBASELL_CONFIRM_REMOVE_AUTO_SELL_POOR_ITEMS_EXCEPTION"] = {
        text = L["AutoSellPoorExceptionRemove"],
        button1 = YES,
        button2 = NO,
        timeout = 0,
	  	whileDead = 1,
   hideOnEscape = 1,
	...
	StaticPopupDialogs["JAMBASELL_CONFIRM_REMOVE_AUTO_SELL_OTHER_ITEMS"] = {
        text = L["AutoOtherPoorExceptionRemove"],
        button1 = YES,
        button2 = NO,
        timeout = 0,
		  whileDead = 1,
	...
	StaticPopupDialogs["JAMBASELL_CONFIRM_DELETE_ITEMS"] = {
  				text = L["AltDeleteItemsPopup"],
	  	 button1 = YES,
	  	 button2 = NO,
		   timeout = 0,
	   whileDead = 1,
  hideOnEscape = 1,
			OnAccept = function()
	          			 AJM:ConfirmTeamDelete(popupLink)
								 end,
	}
	StaticPopupDialogs["JAMBASELL_CONFIRM_DELETE_TOON_ITEMS"] = {
					text = L["AltDeleteItemsOnToonPopup"](popupName),
			 button1 = YES,
			 button2 = NO,
			 timeout = 0,
		 whileDead = 1,
	hideOnEscape = 1,
			OnAccept = function()
									 AJM:ConfirmToonDelete(popupName, popupLink)
								 end,
	   OnDecline = function (args)
						   	   popupDeleteOnToon = false
	   				 	   end,
	}
	StaticPopupDialogs["JAMBASELL_CONFIRM_DELETE_TOONS_ITEMS"] = {
					text = L["AltDeleteItemsOnToonsPopup"],
			 button1 = YES,
			 button2 = NO,
			 timeout = 0,
		 whileDead = 1,
...
function AJM:ContainerFrameItemButton_OnModifiedClick( self, event, ... )
	...
	if IsAltKeyDown() == 1 then
		if AJM.db.sellItemOnAllWithAltKey == true and MerchantFrame:IsVisible() == 1 then
			...
			elseif AJM.db.deleteItemOnAllWithAltKey == true then
				AJM:PopupTeamDelete(event, link)
			end
		elseif AJM.db.deleteItemOnAllWithAltKey == true then
			local bag, slot = self:GetParent():GetID(), self:GetID()
			local texture, count, locked, quality, readable, lootable, link = GetContainerItemInfo( bag, slot )
...
function AJM:PopupTeamDelete( event, link )
	popupLink = link
	StaticPopup_Show( "JAMBASELL_CONFIRM_DELETE_ITEMS" )
end

function AJM:ConfirmTeamDelete(link)
	AJM:JambaSendCommandToTeam( AJM.COMMAND_DELETE_ITEM, link )
end

function AJM:ConfirmToonDelete(characterName, link)
	popupDeleteOnToon = false
	AJM:JambaSendCommandToToon( characterName, AJM.COMMAND_DELETE_ITEM, link )
end

function AJM:DoSellItem( characterName, itemlink )
		...
		for slot = 1, GetContainerNumSlots( bag ) do
			...
			if bagItemLink ~= nil then
				...
				if JambaUtilities:DoItemLinksContainTheSameItem( bagItemLink, itemlink ) then
					local name, link, quality, iLevel, reqLevel, class, subclass, maxStack, equipSlot, texture, vendorPrice = GetItemInfo( bagItemLink )
					if vendorPrice ~= 0 then
						-- Yes, sell this item.
						if MerchantFrame:IsVisible() == 1 then
							UseContainerItem( bag, slot )
							-- Tell the boss.
							AJM:JambaSendMessageToTeam( AJM.db.messageArea, L["ItemSell"]( bagItemLink ) )
						elseif AJM.db.deleteItemOnAllWithAltKey == true then
							AJM:JambaSendCommandToToon(characterName, AJM.CONFIRM_DELETE_ITEM, bagItemLink )
						end
					end
				end
			end
...
function AJM:DoDeleteItem( itemlink )
	-- Iterate each bag the player has.
	for bag = AJM.BAG_PLAYER_BACKPACK, AJM.BAG_PLAYER_MAXIMUM do
		-- Iterate each slot in the bag.
		for slot = 1, GetContainerNumSlots( bag ) do
			...
			if bagItemLink ~= nil then
				...
				if JambaUtilities:DoItemLinksContainTheSameItem( bagItemLink, itemlink ) then
					...
					DeleteCursorItem()
					-- Tell the boss.
					AJM:JambaSendMessageToTeam( AJM.db.messageArea, L["ItemDelete"]( bagItemLink ) )
				end
			end
		end
	end
...
function AJM:SellPoorItemIfNotAnException( itemName, itemLink, bag, slot )
	...
	if canSell == true then
		...
		UseContainerItem( bag, slot )
		AJM:JambaSendMessageToTeam( AJM.db.messageArea, L["ItemSell"]( itemLink ) )
	else
		AJM:JambaSendMessageToTeam( AJM.db.messageArea, L["ItemSellFail"]( itemLink ) )
	end
end
...
function AJM:JambaOnCommandReceived( characterName, commandName, ... )
	...
	if commandName == AJM.COMMAND_SELL_ITEM then
		AJM:DoSellItem( characterName, ... )
	end
	if commandName == AJM.COMMAND_DELETE_ITEM then
		print(...)
		AJM:DoDeleteItem( ... )
	end
	if commandName == AJM.CONFIRM_DELETE_ITEM then
		AJM:PopupToonDelete( _, characterName, ... )
	end
end
