...
function AJM:GetConfiguration()
	local configuration = {
		...
		args = {
			...
			ignore = {
				type = "input",
				name = "Ignore Team wide",
				desc = "Ignores a player across the whole team",
				usage = "/jamba-team ignore <add/del> <player>",
				get = false,
				set = "CommandIgnorePlayer",
			},
		},
	}
...
-------------------------------------------------------------------------------------------------------------
AJM.COMMAND_IGNORE_PLAYER = "JambaTeamIgnorePlayer"
...
local function GetCharacterIndex(name)
...
function AJM:CommandIgnorePlayer( info, parameters )
	local addDel, ignoreName = strsplit( " ", parameters )
  AJM:JambaSendCommandToTeam(AJM.COMMAND_IGNORE_PLAYER, addDel, ignoreName )
end


function AJM:ReceiveIgnorePlayer( addDel, ignoreName )
	if addDel == "add" then
		AddIgnore(ignoreName)
		AJM:Print("Added Ignore:", ignoreName)
	elseif addDel == "del" then
		DelIgnore(ignoreName)
		AJM:Print("Deleted Ignore:", ignoreName)
	end
end
...
local function TeamListOrdered()
	...
	table.sort( AJM.orderedCharacters,
		function( characterA, characterB )
			local positionA = 0
			local positionB = 0
			for key, value in pairs( AJM.db.teamList ) do
				--print(value)
				if key == characterA then
					positionA = value
				end
				if key == characterB then
					positionB = value
				end
			end
...
local function TeamListOrderedOnline()
	...
	table.sort( AJM.orderedCharactersOnline, 
		function( characterA, characterB )
...
local function TeamListOnline()
	JambaUtilities:ClearTable( AJM.charactersOnline )
	for characterName, characterPosition in pairs( AJM.db.teamList ) do
		if JambaApi.GetCharacterOnlineStatus( characterName ) == true then
			table.insert( AJM.charactersOnline, characterName )
		end
	end
	return ipairs( AJM.charactersOnline )
end
...
function AJM:InviteTeamToParty(info, parameters)
	if parameters ~= nil and parameters ~= "" then
		AJM:InviteTeamToPartys(nil, parameters)
		return
	end
	...
	if JambaPrivate.Tag.DoesCharacterHaveTag( AJM.characterName, tag ) == false then
		...
		for index, characterName in TeamListOrdered() do
			...
			if JambaPrivate.Tag.DoesCharacterHaveTag( characterName, tag ) then
				...
				AJM:JambaSendCommandToToon(characterName, AJM.COMMAND_TAG_PARTY, tag )
				break
			end
		end
		return
...
function AJM:TagParty(info, characterName, tag, ...)
	--AJM:Print("test", characterName, tag )
...
function AJM:JambaOnCommandReceived( sender, commandName, ... )
	...
	if commandName == AJM.COMMAND_TAG_PARTY then
		if IsCharacterInTeam( sender ) == true then
			AJM.TagParty( info, nil, tag, ... )
		end
	end
	if commandName == AJM.COMMAND_IGNORE_PLAYER then
		if IsCharacterInTeam( sender ) == true then
			AJM.ReceiveIgnorePlayer( ... )
		end
	end
end
...
JambaPrivate.Team.GetCharacterIndex = GetCharacterIndex
...
JambaPrivate.Team.setOnline = setOnline
...
JambaApi.TeamListOnline = TeamListOnline
