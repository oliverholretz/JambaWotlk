...
-------------------------------------------------------------------------------------------------------------
local CommandPrefix = "JmbTD"
AJM.COMMAND_FOLLOW_STATUS_UPDATE = CommandPrefix .. "Fl"
AJM.COMMAND_EXPERIENCE_STATUS_UPDATE = CommandPrefix .. "XP"
AJM.COMMAND_TOONINFORMATION_UPDATE = CommandPrefix .. "TI"
AJM.COMMAND_REPUTATION_STATUS_UPDATE = CommandPrefix .. "RP"
AJM.COMMAND_COMBO_STATUS_UPDATE = CommandPrefix .. "Cmb"
AJM.COMMAND_REQUEST_INFO = CommandPrefix .. "Inf"
...
local function GetCharacterHeight()
	...
	if AJM.db.barsAreStackedVertically == true then
		...
		height = max( heightPortrait, heightFollowStatus, heightExperienceStatus, heightExperienceStatus, heightHealthStatus, heightPowerStatus )
	end
	return height
end
...
local function GetCharacterWidth()
	...
	if AJM.db.barsAreStackedVertically == true then
		width = widthPortrait + max( widthFollowStatus, widthExperienceStatus, widthReputationStatus, widthHealthStatus, widthPowerStatus )
	else
...
function AJM:CreateJambaTeamStatusBar( characterName, parentFrame )
	...
	followBar.Gold = 0
	followBar.Mail = "MLnothing"
	followBar.CurrText = "currNothing"
	followBar.GearScore = "GSnothing"
	followBar.CharacterLevel = 1
	followBar.MaxCharacterLevel = 80
	...
	reputationBarText:SetTextColor( 1.00, 1.00, 0.00, 1.00 )
	reputationBarText:SetAllPoints()
	reputationBarText.playerReputation = 100
	reputationBarText.playerMinReputation = 100
	reputationBarText.playerMaxReputation = 100
	reputationBarText.playerMaxLevel = 4
	characterStatusBar["reputationBarText"] = reputationBarText
	AJM:UpdateReputationStatus( characterName, 0, 0, 0, 1 )
...
function AJM:ShowFollowTooltip( frame, followBar, characterName, canShow )
	AJM:Debug("ShowFollowTooltip()", characterName, followBar.FreeBagSpace, followBar.TotalBagSpace, followBar.Durability, followBar.Gold, followBar.Mail, followBar.CurrText, followBar.GearScore, followBar.CharacterLevel, followBar.MaxCharacterLevel, canShow )
	AJM:JambaRequestUpdate(characterName)
	--Tooltip
	if canShow then
		if AJM.db.showToolTipInfo == true then
			local combat = UnitAffectingCombat("player")
			if combat == nil then
				AJM:Debug("ShowFollowTooltip()", "combat == nil")
				--followBarClick:SetScript("OnEnter", function(self)
				GameTooltip:SetOwner(frame, "ANCHOR_BOTTOM")
				
				GameTooltip:AddLine(characterName, 1, 0.82, 0, 1)
				...
				if followBar.CharacterLevel ~= followBar.MaxCharacterLevel then
					...
					-- Gearscore
					if not (followBar.GearScore == "GSnothing") then
						GameTooltip:AddLine(L["GearScore:"]..L[" "]..followBar.GearScore ,1,1,1,1)
					end
					-- Gold
					GameTooltip:AddLine(" ",1,1,1,1)
					GameTooltip:AddLine(L["Gold:"]..L[" "]..GetCoinTextureString( followBar.Gold ),1,1,1,1)
					-- Shows if has Ingame Mail
					if not (followBar.Mail == "MLnothing") then
						GameTooltip:AddLine(" ",1,1,1,1)
					...
					if not (followBar.CurrText == "currNothing") then
						GameTooltip:AddLine(" ",1,1,1,1)
						GameTooltip:AddLine("Tracked Currencies:", 1, 0.82, 0, 1)
...
local function SettingsCreateDisplayOptions( top )
	...
	AJM.settingsControl.displayOptionsReputationStatusWidthSlider:SetSliderValues( 5, 200, 1 )
...
function AJM:JambaOnCommandReceived( characterName, commandName, ... )
	...
	if commandName == AJM.COMMAND_REQUEST_INFO then
		AJM:SendInformationUpdateCommand(characterName, ... )
	end
end
...
function AJM:JambaRequestUpdate(characterName)
	if characterName ~= nil then
		AJM:JambaSendCommandToToon(characterName, AJM.COMMAND_REQUEST_INFO, characterName )
	end
end



function AJM:SendInformationUpdateCommand(sender, ...)
	local name = ...
	AJM:Debug("SendInformationUpdateCommand()", name, sender)
	...
	local characterLevel = UnitLevel("player")
		--Max Level
	local characterMaxLevel = 80
		-- gold
	local gold = GetMoney()
	-- GearScore
	local gearscore = "GSnothing"
	if IsAddOnLoaded("GearscoreLite") then
		gearscore, _ = GearScore_GetScore(UnitName("player"), "player");
	end
		-- Bag information
	...
	if ( name ) then
		AJM:Debug("SendInformationUpdateCommand()", "curr1", name, count)
		currText = name.." ".." |T"..icon..":16|t".." "..count
	end
	local name, count, icon, currencyID = GetBackpackCurrencyInfo(2)
	if ( name ) then
		AJM:Debug("SendInformationUpdateCommand()", "curr2", name, count)
		currText = currText.."\n"..name.." ".." |T"..icon..":16|t".." "..count
	end
	local name, count, icon, currencyID = GetBackpackCurrencyInfo(3)
	if ( name ) then
		AJM:Debug("SendInformationUpdateCommand()", "curr3", name, count)
		currText = currText.."\n"..name.." ".." |T"..icon..":16|t".." "..count
	end
	if AJM.db.showTeamListOnMasterOnly == true then
		AJM:JambaSendCommandToMaster( AJM.COMMAND_TOONINFORMATION_UPDATE, characterLevel, characterMaxLevel, gold, gearscore, durability, slotsFree, totalSlots, currText, mailText)
	else
		AJM:JambaSendCommandToTeam( AJM.COMMAND_TOONINFORMATION_UPDATE, characterLevel, characterMaxLevel, gold, gearscore, durability, slotsFree, totalSlots, currText, mailText)
	end
end

function AJM:SettingsUpdateToonInformationUpdateAll()
	for characterName, characterStatusBar in pairs( AJM.characterStatusBar ) do
		AJM:SendInformationUpdateCommand( AJM.characterName, characterName )
	end
end

function AJM:ProcessUpdateToonInformationMessage( characterName, characterLevel, characterMaxLevel, gold, gearscore, durability, slotsFree, totalSlots, currText, mailText )
	AJM:SettingsUpdateToonInformation( characterName, characterLevel, characterMaxLevel, gold, gearscore, durability, slotsFree, totalSlots, currText, mailText )
end


function AJM:SettingsUpdateToonInformationAll()
	for characterName, characterStatusBar in pairs( AJM.characterStatusBar ) do
		AJM:SettingsUpdateToonInformation( characterName, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil )
	end
end

function AJM:SettingsUpdateToonInformation( characterName, characterLevel, characterMaxLevel, gold, gearscore, durability, slotsFree, totalSlots, currText, mailText )
	AJM:Debug("SettingsUpdateToonInformation()", characterName, characterLevel, characterMaxLevel, gold, gearscore, durability, slotsFree, totalSlots, currText, mailText  )
...
function AJM:SendFollowStatusUpdateCommand( isFollowing )
	if AJM.db.showTeamList == true and AJM.db.showFollowStatus == true then
		...
		if JambaApi.Follow ~= nil  then
			if JambaApi.Follow.IsFollowingStrobing() == true or JambaApi.SuppressNextFollowWarningthen == true then
				canSend = false
				AJM:Debug("SendFollowStatusUpdateCommand()", "canSend", canSend)
			end
...
function AJM:ProcessUpdateFollowStatusMessage( characterName, isFollowing )
	AJM:UpdateFollowStatus( characterName, isFollowing, followMaster )
end
...
function AJM:UpdateFollowStatus( characterName, isFollowing, isFollowLeader )
	...
	if isFollowing == true then
		-- Following.
		followBar:SetStatusBarColor( 0.05, 0.85, 0.05, 0.50 )
	else
...
function AJM:UpdateExperienceStatus( characterName, playerExperience, playerMaxExperience, exhaustionStateID )
	...
	local experienceBar = characterStatusBar["experienceBar"]
	if UnitLevel(characterName) ~= nil and UnitLevel(characterName) == 80 then
		experienceBar:SetValue(0)
		experienceBarText:SetText("")
		return
	end
...
function AJM:UPDATE_FACTION( event, ... )
...
function AJM:SendReputationStatusUpdateCommand()
	if AJM.db.showTeamList == true and AJM.db.showReputationStatus == true then
		local _, playerReputationLevel, playerMinReputation, playerMaxReputation, playerReputation = GetWatchedFactionInfo()
		if playerMaxReputation ~= 0 then
			if AJM.db.showTeamListOnMasterOnly == true then
				AJM:JambaSendCommandToMaster( AJM.COMMAND_REPUTATION_STATUS_UPDATE, playerReputation, playerMinReputation, playerMaxReputation, playerReputationLevel )
			else
				AJM:JambaSendCommandToTeam( AJM.COMMAND_REPUTATION_STATUS_UPDATE, playerReputation, playerMinReputation, playerMaxReputation, playerReputationLevel )
			end
		end
...
function AJM:SettingsUpdateReputationAll()
	for characterName, characterStatusBar in pairs( AJM.characterStatusBar ) do
		AJM:UpdateReputationStatus( characterName, nil, nil, nil, 1 )
	end
end

function AJM:ProcessUpdateReputationStatusMessage( characterName, playerReputation, playerMinReputation, playerMaxReputation, playerReputationLevel )
	AJM:UpdateReputationStatus( characterName, playerReputation, playerMinReputation, playerMaxReputation, playerReputationLevel )
end

function AJM:UpdateReputationStatus( characterName, playerReputation, playerMinReputation, playerMaxReputation, playerReputationLevel )
	if CanDisplayTeamList() == false then
		return
	end
	...
	if playerReputation == nil then
	   playerReputation = reputationBarText.playerReputation
	end
	if playerMinReputation == nil then
	   playerMinReputation = reputationBarText.playerMinReputation
	end
	if playerMaxReputation == nil then
		playerMaxReputation = reputationBarText.playerMaxReputation
	end
	if playerReputationLevel == nil then
		playerReputationLevel = reputationBarText.playerReputationLevel
	end

	reputationBarText.playerMinReputation = playerMinReputation
	reputationBarText.playerReputation = playerReputation
	reputationBarText.playerMaxReputation = playerMaxReputation
	reputationBarText.playerReputationLevel = playerReputationLevel

	reputationBar:SetMinMaxValues( 0, tonumber( playerMaxReputation) - tonumber( playerMinReputation ) )
	reputationBar:SetValue( tonumber( playerReputation - playerMinReputation ) )

	local color = FACTION_BAR_COLORS[tonumber( playerReputationLevel )];
	reputationBar.backgroundTexture:SetTexture( color.r, color.g, color.b, 0.50 )


	local text = ""
	if AJM.db.reputationStatusShowValues == true then
		text = text..tostring( playerReputation - playerMinReputation )..L[" / "]..tostring( playerMaxReputation - playerMinReputation )..L[" "]
	end
	if AJM.db.reputationStatusShowPercentage == true then
		if AJM.db.reputationStatusShowValues == true then
			text = text..L["("]..tostring( floor( ((playerReputation - playerMinReputation)/(playerMaxReputation - playerMinReputation))*100) )..L["%"]..L[")"]
		else
			text = tostring( floor( ((playerReputation - playerMinReputation)/(playerMaxReputation - playerMinReputation))*100) )..L["%"]
		end
	end
...
function AJM:OnEnable()
	...
	AJM:RegisterEvent( "UPDATE_FACTION" )
	...
	AJM:ScheduleTimer( "SendReputationStatusUpdateCommand", 20 )
	AJM:ScheduleTimer( "SettingsUpdateToonInformationUpdateAll", 20 )
